...
{
    word coalescenceKernelType(dict.lookup("coalescenceKernel"));

    Info<< "Selecting coalescenceKernel "
        << coalescenceKernelType << endl;
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            ...
            << "Valid coalescenceKernelType types are :" << endl
            << dictionaryConstructorTablePtr_->sortedToc()
            << abort(FatalError);
    }

    return autoPtr<coalescenceKernel>(cstrIter()(dict));
}
