...
#include "solidContentNucleation.H"
...
{
    defineTypeNameAndDebug(solidContentNucleation, 0);

    addToRunTimeSelectionTable
    (
        nucleationModel,
        solidContentNucleation,
        dictionary
    );
}
}
...
Foam::populationBalanceSubModels::nucleationModels::solidContentNucleation::solidContentNucleation
(
...
Foam::scalar
Foam::populationBalanceSubModels::nucleationModels::solidContentNucleation::nucleationSource
(
...
) const
{
    ...
    const scalar inhibition =
        1.0/(1.0 + std::pow(m3_[celli]/m3Crit_, r_));

    const scalar J =
        kb_.value()
      * std::pow(max(sigmaVal, scalar(0)), powCoeff_)
      * inhibition;
