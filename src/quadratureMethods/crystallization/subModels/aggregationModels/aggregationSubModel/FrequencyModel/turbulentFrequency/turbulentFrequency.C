...
{
    defineTypeNameAndDebug(turbulentFrequency, 0);
...
crystalCollisionFrequencies::turbulentFrequency::turbulentFrequency
(
...
:
    ...
    Cturb_(dict.lookupOrDefault<scalar>("Cturb", 1.522)),  // 1.91 * 0.797
    nu_
    (
...
Foam::scalar
Foam::populationBalanceSubModels::aggregationKernels::
crystalCollisionFrequencies::turbulentFrequency::beta
(
    const scalar& d1,
    const scalar& d2,
    const vector& Ur,
...
) const
{
    ...
    // Calculate turbulent collision frequency
    // beta_turb = C_turb * sqrt(epsilon/nu) * (L1 + L2)^3
    const scalar beta = Cturb_ * Foam::sqrt(eps / nu) * pow3(d1 + d2);
